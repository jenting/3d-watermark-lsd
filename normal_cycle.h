...
void principal_curvature(Polyhedron &pMesh,bool IsGeod,double radius)
{
      ...
      pVertex++)
  {
	...
	//valeurs propres
			...
			pMesh.MinNrmMinCurvature(min(pMesh.MinNrmMinCurvature(),pVertex->Kmin()));
			pMesh.MaxNrmMinCurvature(max(pMesh.MaxNrmMinCurvature(),pVertex->Kmin()));

			pMesh.MinNrmMaxCurvature(min(pMesh.MinNrmMaxCurvature(),pVertex->Kmax()));
			pMesh.MaxNrmMaxCurvature(max(pMesh.MaxNrmMaxCurvature(),pVertex->Kmax()));
  }
                                       ...
                                       double ppMatrix_sum[3][3])
{
  ...
  CGAL_For_all(pHalfedge,pHalfedgeStart)
  {
    ...
    // build edge vector and comput its norm
	  ...
	  double len_edge = sqrt(edge*edge);
